...
-- ── Query 1: Overall Business Summary (KPI Dashboard) ────────
...
-- ── Query 6: Ranking Orders with ROW_NUMBER (Per Customer) ───
...
-- ── Query 7: Global Revenue Ranking — RANK vs DENSE_RANK ─────
SELECT order_id,
       customer_id,
       total_amount,
...
-- ── Query 9: Customer Value Segmentation with CTEs ───────────
...
combined AS (
  SELECT c.name, c.city,
         s.total_sessions, s.total_pages,
...
SELECT name, city, total_sessions, total_pages,
       avg_duration, total_orders, total_spent,
       ROUND(
...
-- ── EXPLAIN ANALYZE — Query 3 (Performance Check) ────────────
EXPLAIN ANALYZE
SELECT TO_CHAR(order_date, 'YYYY-MM') AS month,
       COUNT(*) AS num_orders,
       SUM(total_amount) AS monthly_revenue
...
-- ── EXPLAIN ANALYZE — Query 9 (CTE Performance Check) ────────
EXPLAIN ANALYZE
WITH customer_spend AS (
...
SELECT tier, COUNT(*), ROUND(SUM(total_spent), 2)
FROM customer_tiers
GROUP BY tier
ORDER BY 2 DESC;
